...
(defun concat-sequence (&args producers)
...
(defun piano (input)
  (lam-let ((envelope (adsr-envelope input (seconds 0.01) (seconds 0.2) 0.3 (seconds 0.3)))
            (oscillator (make-sine-wave (constant 440))))
           (* oscillator (or envelope 0))))

(defun piano-notes ()
  (let ((note-sequence (append (make-list 30000 :initial-element 1))))
...
(defmacro while (test &rest forms)
  `(do nil ((not ,test)) ,@forms))

(defun make-thread (&rest forms))
...
(defun start-output ()
  ...
  (bt:make-thread
   (lambda ()
     (while *running* (format t "step") (sleep 2)))
   :name "bla-thread"))

(defun stop-output ()
  (setf *running* nil))

(setf buffer1 (make-buffer +buffer-size+))
(setf buffer2 (make-buffer +buffer-size+))
(setf lock1 (bt:make-lock))
(setf lock2 (bt:make-lock))

(defun start-output ()
  (setf *running* t)
  (bt:make-thread
   (lambda ()
     (let ((bufs (list buffer1 buffer2))
           (locks (list lock1 lock2)))
       (while *running*
              (bt:acquire-lock (first locks))
              (sleep 1)
              (print "OK")
              (bt:release-lock (first locks))
              (rotatef (first bufs) (second bufs))
              (rotatef (first locks) (second locks)))))

   :name "bla-thread"))

(defvar *synth-running* nil)

(defun start-synth ()
  (unless *synth-running*
    (setf *synth-running* t)))



(defun stop-synth ()
  (setf *synth-running* nil))

(defun asd ()
  (start-output)
  (stop-output))

(defun rot-left (lst) (nconc (cdr lst) (list (car lst))))
